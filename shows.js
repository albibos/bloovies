document.addEventListener("DOMContentLoaded", function () {
  ...
  if (
    document.getElementById("blazer") &&
    document.getElementById("blazer").innerHTML ===
      'Official <span>Blazer</span> Service'
  ) {
    if (urlParams.get("episode")) {
      ...
      // Add this line to define the video variable
      const video = document.getElementById("player");

      if (Hls.isSupported() && episodeId && mediaId) {
        var hls = new Hls();
        fetch(
          `https://api.consumet.org/movies/flixhq/watch?episodeId=${episodeId}&mediaId=${mediaId}`
        )
    ...
    document.querySelector("#location").innerHTML =
      window.location.href + "?movie=watch-puss-in-boots-the-last-wish-91342";
